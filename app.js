...
function render(){setNav(state.screen);if(state.role!=='coach'){root.innerHTML='<div class="page-title"><h1>'+esc(state.role.charAt(0).toUpperCase()+state.role.slice(1))+' Hub</h1><p>This role shell is ready for its own screens. Coach screens remain separate.</p></div>';syncBackButton();return}if(state.screen==='home')renderHome();else if(state.screen==='schedule')renderSchedule();else if(state.screen==='resources')renderResources();else if(state.screen==='venues')renderVenues();else if(state.screen==='venue-detail')renderVenueDetail(state.selectedVenue);else if(state.screen==='support')renderSupport();else if(state.screen==='more')renderMore();else if(state.screen==='management')renderManagement();syncBackButton()}
function renderHome(){
...
function renderResources(){
 ...
 root.innerHTML='<div class="page-title"><h1>Resources</h1><p>Session plans, curriculum, drills and useful documents.</p></div><div class="resource-grid">'+
  (list.length?list.map(function(r){
   ...
   var body='<div class="resource-img'+(img?' has-image':'')+'">'+(img||esc(r.title))+'</div><div class="resource-body">'+
...
function openSupportDetail(id){
 ...
 sheetContent.innerHTML='<div class="calendar-sheet"><h3>'+esc(s.title)+'</h3>'+
  ...
  '<p style="white-space:pre-wrap;color:var(--ink)">'+esc(s.body||'')+'</p>'+
  (s.external_link?'<a class="primary-btn" href="'+esc(s.external_link)+'" target="_blank" rel="noopener" style="display:block;text-align:center;text-decoration:none;margin-top:10px">Open link</a>':'')+

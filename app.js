 ...
 */
var panes={};
var TAB_SCREENS={home:1,schedule:1,resources:1,players:1};
 ...
 */
function weekChangesSummary(days){
...
function renderWeekSchedule(){var start=addDays(allowedScheduleStart(),state.scheduleWeekOffset*7),end=addDays(start,6),days=workingDaysForWeek(start),today=new Date(),isCurrent=state.scheduleWeekOffset===0;return weekChangesSummary(days)+'<section class="week-nav"><button data-action="week-shift" data-dir="-1" '+(state.scheduleWeekOffset===0?'disabled':'')+'>‹</button><b>'+esc(formatDateShort(start))+' – '+esc(formatDateShort(end))+'</b><button data-action="week-shift" data-dir="1" '+(state.scheduleWeekOffset===3?'disabled':'')+'>›</button></section><section class="week-stack">'+(days.length?days.map(function(g){var key=iso(g.date),open=(isCurrent&&sameDay(g.date,today))||state.expandedDay===key;return '<article class="week-day '+(open?'is-open':'')+'"><button class="week-day-head" data-action="toggle-day" data-date="'+key+'"><span><b>'+esc(formatDateLong(g.date))+'</b><small>'+g.list.length+' session'+(g.list.length===1?'':'s')+'</small></span><span>'+(open?'⌃':'⌄')+'</span></button>'+(open?'<div class="week-day-list">'+g.list.map(function(o){return scheduleCard(o,true)}).join('')+'</div>':'')+'</article>'}).join(''):'<div class="schedule-empty">No sessions this week.</div>')+'</section><button class="calendar-action" data-action="calendar-options" data-scope="week" data-date="'+iso(start)+'">▣ Add this week to Calendar <span>⌄</span></button>'}
